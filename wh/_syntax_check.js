...
// 按 _idx 查找题目（本站数据分笔试/面试/兜底三池）
function szQuestionByIdx(idx) {
  var pools = [W_DATA, I_DATA, G_DATA];
  for (var p = 0; p < pools.length; p++) {
    for (var i = 0; i < pools[p].length; i++) {
      if (pools[p][i]._idx === idx) return pools[p][i];
    }
  }
  return null;
}
// 笔试选项点击选中（单选清兄弟，多选切换）
function szToggleOpt(el) {
  var card = el.closest(".card"); if (!card) return;
  var isMulti = card.getAttribute("data-multi") === "1";
  if (!isMulti) card.querySelectorAll(".opt-row.selected").forEach(function (o) { o.classList.remove("selected"); });
  el.classList.toggle("selected");
}
// 笔试「核对答案」：显示答案 + 判定横幅
function szRevealAnswer(idx) {
  try {
    var q = szQuestionByIdx(idx); if (!q) return;
    var card = document.getElementById("q" + idx); if (!card) return;
    var ansEl = document.getElementById("ans" + idx); if (!ansEl) return;
    if (ansEl.style.display === "block") { ansEl.style.display = "none"; return; }
    var sel = [];
    card.querySelectorAll(".opt-row.selected").forEach(function (o) { sel.push(o.getAttribute("data-letter")); });
    if (!sel.length) {
      var b = card.querySelector(".check-btn");
      if (b) { var t = b.textContent; b.textContent = "⚠️ 请先选择答案"; setTimeout(function () { b.textContent = t; }, 1200); }
      return;
    }
    card.querySelectorAll(".opt-row").forEach(function (o) {
      ...
      var s = o.classList.contains("selected");
      if (s) o.classList.add(isCorr ? "ok" : "bad"); else if (isCorr) o.classList.add("right");
    });
    ansEl.style.display = "block";
    var oldB = ansEl.querySelector(".ans-result"); if (oldB) oldB.remove();
    var isCorrect;
    if (q.type === "multiple" || q.type === "multi") {
      var a = normAnsArr(q.answer).slice().sort().join("");
      isCorrect = sel.slice().sort().join("") === a && sel.length > 0;
    } else {
...
// 拆分笔试 batch 为批次名与题型标签（批次筛选不再和题型混在一起）
[W_DATA, G_DATA].forEach(function(list){
  list.forEach(function(q){
    if (!q || q._batchName !== undefined) return;
    var raw = String(q.batch || "");
    var parts = raw.split("·");
    q._batchName = parts[0] || raw || "未注明";
    q._typeLabel = (parts[1] || WRITTEN_TYPE_LABEL[q.type] || q.type || "未知").trim();
  });
});
...
// 默认模式：两库都在时，面试优先展示（笔试已结束）；否则展示存在的那个
var MODE = (hasW && hasI)
  ? (CONFIG.defaultPage === "written" ? "written" : "interview")
...
function activeDimensions() {
  return MODE === "interview"
    ...
    : [{ key: "_batchName", label: "批次" }, { key: "type", label: "题型" }];
}
function deriveValues(dim) {
  ...
  data.forEach(function (q) {
    ...
    else if (dim === "_batchName") { if (q._batchName) set[q._batchName] = 1; }
...
function groupKey(q) { return MODE === "interview" ? (q.year || "未注明年份") : (q._batchName || "未注明批次"); }
...
function cardHtml(q) {
  ...
  if (MODE === "interview") {
    ...
    bodyInner = answerSection(formatAnswer(q.answer || "（暂无参考答案）", currentSearch));
  } else {
    var wl = WRITTEN_TYPE_LABEL[q.type] || q.type;
    badges = (q.num != null ? '<span class="badge badge-year">#' + q.num + "</span>" : "")
      + (q._batchName ? '<span class="badge badge-session">' + escapeHtml(q._batchName + "·" + q._typeLabel) + "</span>" : "")
    ...
    if (q.options && q.options.length) {
      q.options.forEach(function (o) {
        optsHtml += '<div class="opt-row selectable" data-letter="' + escapeHtml(o.letter) + '" onclick="szToggleOpt(this)"><span class="opt-letter">' + escapeHtml(o.letter) + ".</span><span>" + escapeHtml(o.text) + "</span></div>";
      });
    }
    ...
    var checkBtn = (q.options && q.options.length) ? '<button class="check-btn" onclick="szRevealAnswer(' + q._idx + ')">📋 核对答案</button>' : "";
    bodyInner = (optsHtml ? '<div style="margin:12px 0">' + optsHtml + "</div>" : "")
      + '<div class="answer-wrap" id="ans' + q._idx + '" style="display:none">' + answerSection(ansHtml) + '</div>'
      + '<div class="check-row">' + checkBtn + '</div>';
  }
  var isMulti = (q.type === "multiple" || q.type === "multi") ? "1" : "0";
  var statusDB = StorageCtrl.get();
  var cur = statusDB[q._idx] || "not-mastered";
  var actNM = cur === "not-mastered" ? "active" : "";
  var actM = cur === "mastered" ? "active" : "";
  return '<div class="card" id="q' + q._idx + '" data-multi="' + isMulti + '"><div class="card-header"><div class="card-left"><div class="card-badges">' + badges + '</div><div class="card-title">' + titleHtml + '</div></div><span class="arrow">▼</span></div>'
...
function render() {
  if (document.getElementById("examBtn")) document.getElementById("examBtn").style.display = (MODE === "interview") ? "none" : "";
...
function updateStats() {
  var data = dataset();
  var dimKey = MODE === "interview" ? "year" : "_batchName";
  var dimSet = {}, typeSet = {};
  ...
  document.getElementById("statLabel0").textContent = "总题数";
...
function buildModeTabs() {
  ...
  box.innerHTML = "";
  var tabs = [];
  if (hasI || singleMode === "interview") tabs.push({ m: "interview", label: "🎤 面试" });
  if (hasW || singleMode === "written") tabs.push({ m: "written", label: "📖 笔试" });
...
var randomModalReveal = document.getElementById("randomModalReveal");
...
function randomQuestionHtml(q) {
  ...
  if (q.options && q.options.length) {
    ...
    q.options.forEach(function (o) {
      h += '<div class="opt-row selectable" data-letter="' + escapeHtml(o.letter) + '" onclick="szRndToggleOpt(this)"><span class="opt-letter">' + escapeHtml(o.letter) + ".</span><span>" + escapeHtml(o.text) + "</span></div>";
    });
    h += "</div>";
  }
  return h;
...
function randomAnswerHtml(q) {
  if (MODE === "interview") return formatAnswer(q.answer || "（暂无参考答案）", "");
...
function showRandomQuestion() {
  ...
  var ansZone = document.getElementById("randomModalAnswer");
  var hintZone = document.getElementById("randomModalHint");
  ansZone.classList.remove("show"); hintZone.style.display = "flex";
  randomModalReveal.textContent = "👀 点击查看答案";
  randomModal.classList.add("show");
}
document.getElementById("randomBtn").addEventListener("click", showRandomQuestion);
randomModalReveal.addEventListener("click", function () {
  var ans = document.getElementById("randomModalAnswer"), hint = document.getElementById("randomModalHint");
  if (ans.classList.contains("show")) { ans.classList.remove("show"); randomModalReveal.textContent = "👀 点击查看答案"; hint.style.display = "flex"; return; }
  var q = currentRandomQ; if (!q) return;
  if (!rndSelected.length) { hint.style.display = "flex"; hint.textContent = "⚠️ 请先选择你的答案，再点查看"; return; }
  document.querySelectorAll("#randomModalQuestion .opt-row").forEach(function (o) {
    var L = o.getAttribute("data-letter");
    var isCorr = q.answer != null && String(q.answer).indexOf(L) !== -1;
    var s = o.classList.contains("selected");
    if (s) o.classList.add(isCorr ? "ok" : "bad"); else if (isCorr) o.classList.add("right");
  });
  var content = document.getElementById("randomModalAnswerContent");
  var isCorrect;
  if (q.type === "multiple" || q.type === "multi") { var a = normAnsArr(q.answer).slice().sort().join(""); isCorrect = rndSelected.slice().sort().join("") === a && rndSelected.length > 0; }
  else { isCorrect = rndSelected.length === 1 && String(rndSelected[0]).trim() === String(q.answer).trim(); }
  var oldB = content.querySelector(".ans-result"); if (oldB) oldB.remove();
  content.insertBefore(makeResultBanner(isCorrect, q), content.firstChild);
  ans.classList.add("show"); randomModalReveal.textContent = "🙈 收起答案"; hint.style.display = "none";
});
function szRndToggleOpt(el) {
  var q = currentRandomQ; if (!q) return;
  var isMulti = (q.type === "multiple" || q.type === "multi");
  if (!isMulti) { document.querySelectorAll("#randomModalQuestion .opt-row.selected").forEach(function (o) { o.classList.remove("selected"); }); rndSelected = []; }
  el.classList.toggle("selected");
  var L = el.getAttribute("data-letter");
  if (el.classList.contains("selected")) { if (rndSelected.indexOf(L) === -1) rndSelected.push(L); }
  else { rndSelected = rndSelected.filter(function (x) { return x !== L; }); }
}
...
(function () {
  ...
  function setFocus(on) {
    ...
    focusBtn.textContent = on ? "🖥️ 退出全屏" : "🖥️ 全屏刷题";
  ...
  var fv = null;
  try { fv = localStorage.getItem("focusMode"); } catch (e) {}
  if (fv) setFocus(true);
})();
...
function scopeKey(){ return (MODE==="interview") ? "year" : "_batchName"; }
...
// 按范围+题量生成一套试卷：随机打乱，单选/多选/判断按原比例对应
function buildExamPaper(scopeVal, countVal){
  var data = dataset();
  var key = scopeKey();
  if(scopeVal==="__random__"){
    var distinct=[]; var s2={}; data.forEach(function(q){var v=String(q[key]||"未注明"); if(!s2[v]){s2[v]=1;distinct.push(v);}});
    scopeVal = distinct.length ? distinct[Math.floor(Math.random()*distinct.length)] : "";
  }
  var pool = data;
  if(scopeVal && scopeVal!=="__all__"){ pool = data.filter(function(q){ return String(q[key]||"")===String(scopeVal); }); }
  var types=[], seen={}, typeCount={};
  pool.forEach(function(q){ var t=normType(q.type); if(!seen[t]){seen[t]=1;types.push(t);} typeCount[t]=(typeCount[t]||0)+1; });
  var totalPool = pool.length;
  var target = (countVal && countVal>0 && countVal<totalPool) ? countVal : totalPool;
  // 按题型比例抽样，保证 单选/多选/判断 题数对应
  var per={}, assigned=0, frac=[];
  types.forEach(function(t){ var exact=target*typeCount[t]/totalPool; per[t]=Math.floor(exact); assigned+=per[t]; frac.push({t:t,f:exact-Math.floor(exact)}); });
  frac.sort(function(a,b){return b.f-a.f;});
  var k=0; while(assigned<target){ per[frac[k%frac.length].t]++; assigned++; k++; }
  var chosen=[];
  types.forEach(function(t){
    var items = pool.filter(function(q){ return normType(q.type)===t; });
    shuffleArr(items);
    chosen = chosen.concat(items.slice(0, per[t]));
  });
  shuffleArr(chosen);
  return { list: chosen, meta:{ scope:scopeVal, totalPool:totalPool, target:target, per:per } };
}
...
function populateExamSetup(){
  var sel = document.getElementById("examScopeSel");
  var data = dataset();
  var key = scopeKey();
  var vals=[]; var seen={};
  data.forEach(function(q){ var v=String(q[key]||"未注明"); if(!seen[v]){seen[v]=1;vals.push(v);} });
  var opts = '<option value="__random__">🎲 随机抽一套（按批次）</option>'
          + '<option value="__all__">📚 全库随机抽题</option>';
  vals.forEach(function(v){ var c=data.filter(function(q){return String(q[key]||"未注明")===v;}).length; opts += '<option value="'+escapeHtml(v)+'">'+escapeHtml(scopeLabel(v))+'（'+c+'题）</option>'; });
  sel.innerHTML = opts;
  refreshCountOptions();
  sel.onchange = refreshCountOptions;
}
function refreshCountOptions(){
  var scopeSel = document.getElementById("examScopeSel");
  var countSel = document.getElementById("examCountSel");
  var scopeVal = scopeSel.value;
  var data = dataset(); var key = scopeKey();
  var pool = (scopeVal && scopeVal!=="__all__" && scopeVal!=="__random__") ? data.filter(function(q){return String(q[key]||"")===String(scopeVal);}) : data;
  var total = pool.length;
  var presets = [total, 90, 60, 50, 30, 20].filter(function(n,i,a){ return n>0 && a.indexOf(n)===i; }).sort(function(a,b){return b-a;});
  var html = '<option value="0">全量（'+total+'题）</option>';
  presets.forEach(function(n){ if(n!==total) html += '<option value="'+n+'">随机抽 '+n+' 题</option>'; });
  countSel.innerHTML = html;
  var tc={}; pool.forEach(function(q){ var t=normType(q.type); tc[t]=(tc[t]||0)+1; });
  document.getElementById("examSetupInfo").innerHTML = '范围共 '+total+' 题　·　题型构成：'+typesText(tc);
}
function confirmExamSetup(){
  var scopeVal = document.getElementById("examScopeSel").value;
  var countVal = parseInt(document.getElementById("examCountSel").value,10)||0;
  examLastCfg = { scope:scopeVal, count:countVal };
  document.getElementById("examSetupOverlay").style.display = "none";
  enterExamView();
  var paper = buildExamPaper(scopeVal, countVal);
...
function startExam() {
  ...
  document.getElementById("examSubInfo").innerHTML = typesText(meta.per);
  var arr = list.slice();
...
function buildExamCard(q, n) {
  ...
  card.innerHTML = '<div class="exam-q-head"><span class="exam-q-num">#' + n + '</span><span class="exam-q-stem">' + stemHtml + '</span></div>' + bodyHtml;
  return card;
}
...
function submitExam() {
  ...
  if (MODE === "interview") {
    ...
    examResult.innerHTML = '<div class="exam-result-score">' + correct + ' / ' + total + '</div>'
      + '<div class="exam-result-detail">正确率 <b>' + pct + '%</b>　' + grade + '　（答错 ' + wrong + ' 题，已用 ❌ 标出）</div>';
  }
  examResult.style.display = "block";
  window.scrollTo({ top: 0 });
}
function resetExam() {
  ...
  if (examLastCfg) { var paper = buildExamPaper(examLastCfg.scope, examLastCfg.count); examPaperList = paper.list; examPaperMeta = paper.meta; }
...
if (_footer) {
  ...
  var _fp = document.createElement('p'); _fp.id = 'footerText';
  _fp.innerHTML = CONFIG.footerText + (CONFIG.contact ? ' · 联系：' + CONFIG.contact : '');
  _footer.appendChild(_fp);
